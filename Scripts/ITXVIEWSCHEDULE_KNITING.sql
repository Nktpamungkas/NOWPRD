...
WITH QTY_PRODUC_YESTERDAY AS (
	...
	WHERE
		...
		AND TRANSACTIONDATE = 
			CAST(CURRENT_TIMESTAMP AS DATE) - 1 DAY
	GROUP BY
		PROJECTCODE,
		DECOSUBCODE01,
    	DECOSUBCODE02,
...
TOTAL_QTY_DEMAND AS (
	...
	FROM
		PRODUCTIONDEMAND p2
	LEFT JOIN ADSTORAGE a4 ON a4.UNIQUEID = p2.ABSUNIQUEID AND a4.FIELDNAME = 'OriginalPDCode'
	WHERE 
...
TOTAL_MACHINE AS (
	...
	FROM 
		PRODUCTIONDEMAND p3
	LEFT JOIN ADSTORAGE machine ON machine.UNIQUEID = p3.ABSUNIQUEID AND machine.FIELDNAME = 'MachineNoCode'
...
SELECT  
	...
	tqd.TOTAL_QTY - QPA.QTY_PRODUC_ALL AS BELUM_RAJUT,
	CASE
	...
	(tqd.TOTAL_QTY - QPA.QTY_PRODUC_ALL) / (TM.JUMLAH_MACHINE * STD_QTY_PERMACHINE) AS ESTIMATED_COMPLETED,
	CAST((tqd.TOTAL_QTY - QPA.QTY_PRODUC_ALL) / (TM.JUMLAH_MACHINE * STD_QTY_PERMACHINE) AS INT) DAY + CAST(CURRENT_TIMESTAMP AS DATE) - 1 DAY AS ESTIMATED_DELIVERY,
	LISTAGG(DISTINCT DELIVERY_ACTUAL, ', ') AS DELIVERY_ACTUAL,
	SUBCODE01,
	SUBCODE02,
	SUBCODE03,
	SUBCODE04
FROM(
	...
	WHERE
		...
		AND a4.VALUESTRING IS NULL
--		AND p.ORIGDLVSALORDLINESALORDERCODE = 'DOM2500134'
--		AND p.ORIGDLVSALORDLINESALORDERCODE = 'OPN2500093'
--		AND p.ORIGDLVSALORDLINESALORDERCODE = 'S2500061'
	ORDER BY 
		p.ORIGDLVSALORDLINESALORDERCODE DESC) DATAMAIN
LEFT JOIN QTY_PRODUC_YESTERDAY QPY ON QPY.PROJECTCODE = DATAMAIN.PROJECT
...
LEFT JOIN QTY_PRODUC_ALL QPA ON QPA.PROJECTCODE = DATAMAIN.PROJECT
...
LEFT JOIN TOTAL_QTY_DEMAND tqd ON tqd.ORIGDLVSALORDLINESALORDERCODE = DATAMAIN.PROJECT 
...
LEFT JOIN TOTAL_MACHINE TM ON TM.ORIGDLVSALORDLINESALORDERCODE = DATAMAIN.PROJECT 
