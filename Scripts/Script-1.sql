...
--    COALESCE(ITXVIEWRESEP2.GROUPNUMBER, ITXVIEWRESEP.GROUPNUMBER) AS GROUPNUMBER,
    ...
    CASE
        WHEN ITXVIEWRESEP.RECIPETYPE = '1' THEN CAST( ((ITXVIEWRESEP.PICKUPPERCENTAGE/100 * 408.9) + ITXVIEWRESEP.RESIDUALBATHVOLUME) * ITXVIEWRESEP2.CONSUMPTION AS DECIMAL(18, 7))
        ELSE 
            CASE
                WHEN ITXVIEWRESEP2.CONSUMPTIONTYPE = '1' THEN CAST( ((408.9 * VIEWPRODUCTIONRESERVATION.PICKUPQUANTITY) * ITXVIEWRESEP2.CONSUMPTION) / 1000 AS DECIMAL(18, 7))
                WHEN ITXVIEWRESEP2.CONSUMPTIONTYPE = '2' THEN CAST( ((408.9 * (ITXVIEWRESEP2.CONSUMPTION/100)) * 1000) / 1000 AS DECIMAL(18, 7))
            END
    END AS QTY,
    CASE 
...
WHERE 
    ...
    AND VIEWPRODUCTIONRESERVATION.GROUPLINE IN ('303') 
ORDER BY
