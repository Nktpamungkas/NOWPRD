...
FROM
	...
	SELECT
		...
		SELECT
			COUNT(HOLIDAYDATE)
		FROM
			HOLIDAYCALENDARDETAIL h
		WHERE
			h.HOLIDAYDATE BETWEEN CREATION AND TGL_TARGET
        ) AS LIBUR_MINGGU,
		DAYOFWEEK_ISO(TGL_TARGET) AS DayOfWeek,
		CASE
			WHEN DAYOFWEEK_ISO(TGL_TARGET) = 5 THEN
   				DATE(TGL_TARGET) + ((
					SELECT
						COUNT(HOLIDAYDATE)
					FROM
			...
			WHEN 
				DAYOFWEEK_ISO(TGL_TARGET) = 4 
				AND (SELECT COUNT(HOLIDAYDATE) FROM HOLIDAYCALENDARDETAIL h	WHERE h.HOLIDAYDATE BETWEEN CREATION AND TGL_TARGET) < 2
				AND (SELECT	COUNT(HOLIDAYDATE) FROM	HOLIDAYCALENDARDETAIL h	WHERE h.HOLIDAYDATE BETWEEN CREATION AND TGL_TARGET) > 0 THEN
   			 		DATE(TGL_TARGET) + ((
						SELECT
							COUNT(HOLIDAYDATE)
						FROM
			...
			WHEN DAYOFWEEK_ISO(TGL_TARGET) = 4
			AND (SELECT	COUNT(HOLIDAYDATE) FROM	HOLIDAYCALENDARDETAIL h	WHERE h.HOLIDAYDATE BETWEEN CREATION AND TGL_TARGET) >= 2
			AND (SELECT	COUNT(HOLIDAYDATE) FROM	HOLIDAYCALENDARDETAIL h	WHERE h.HOLIDAYDATE BETWEEN CREATION AND TGL_TARGET) > 0 THEN
				    DATE(TGL_TARGET) + ((
							SELECT
			...
			WHEN DAYOFWEEK_ISO(TGL_TARGET) IN (3) THEN
			    DATE(TGL_TARGET) + ((
						SELECT
							COUNT(HOLIDAYDATE)
						FROM
			...
			WHEN DAYOFWEEK_ISO(TGL_TARGET) IN (1, 2) THEN
			    DATE(TGL_TARGET) + ((
						SELECT
							COUNT(HOLIDAYDATE)
						FROM
