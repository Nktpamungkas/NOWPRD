...
SELECT
    ...
    PRODUCTIONDEMAND.PROJECTCODE AS PROJECTCODE_DEMAND,
    ...
    a51.VALUESTRING AS PROD_ORDER_AKJ4,
    a52.VALUESTRING AS SALESORDER_AKJ4,
    a53.VALUESTRING AS PROD_ORDER_AKJ5,
    a54.VALUESTRING AS SALESORDER_AKJ5,
    ...
    SUBSTR(PRODUCTIONDEMAND.CREATIONDATETIME, 1, 10) || ' ' || SUBSTR(PRODUCTIONDEMAND.CREATIONDATETIME, 12, 8) AS CREATIONDATETIME_DEMAND,
     SUBSTRING(
        ad.OPTIONS, 
        POSITION(';' || a47.VALUESTRING || '=' IN ad.OPTIONS) + LENGTH(a47.VALUESTRING) + 2,
        POSITION(';' IN SUBSTRING(ad.OPTIONS, POSITION(';' || a47.VALUESTRING || '=' IN ad.OPTIONS) + LENGTH(a47.VALUESTRING) + 2)) - 1
    ) || ' | ' ||
    SUBSTRING(
        ad2.OPTIONS, 
        POSITION(';' || a48.VALUESTRING || '=' IN ad2.OPTIONS) + LENGTH(a48.VALUESTRING) + 2,
        POSITION(';' IN SUBSTRING(ad2.OPTIONS, POSITION(';' || a48.VALUESTRING || '=' IN ad2.OPTIONS) + LENGTH(a48.VALUESTRING) + 2)) - 1
    )AS KET_AKJ,
    COALESCE(a49.VALUESTRING, ' ') AS NOTE_TAS_KGF,
    a50.VALUESTRING AS LOTBENANGGREIGE
FROM
...
LEFT JOIN (
        SELECT
            DISTINCT 
	a.VALUESTRING AS GROUPING_DYE,
            u.CODE
        FROM
            USERGENERICGROUP u
        LEFT JOIN ADSTORAGE a ON
            a.UNIQUEID = u.ABSUNIQUEID
            AND a.FIELDNAME = 'GroupingDyeing'
        WHERE
            u.USERGENERICGROUPTYPECODE = 'S06'
    ) DYE ON
    DYE.CODE = SALESORDERLINE.SUBCODE01
...
LEFT JOIN ADSTORAGE a51 ON
    a51.UNIQUEID = SALESORDERLINE.ABSUNIQUEID
    AND a51.FIELDNAME = 'PrevProdOrder4Code'
LEFT JOIN ADSTORAGE a52 ON
    a52.UNIQUEID = SALESORDERLINE.ABSUNIQUEID
    AND a52.FIELDNAME = 'PrevSalesOrder4Code'
LEFT JOIN ADSTORAGE a53 ON
    a53.UNIQUEID = SALESORDERLINE.ABSUNIQUEID
    AND a53.FIELDNAME = 'PrevProdOrder5Code'
LEFT JOIN ADSTORAGE a54 ON
    a54.UNIQUEID = SALESORDERLINE.ABSUNIQUEID
    AND a54.FIELDNAME = 'PrevSalesOrder5Code'
...
LEFT JOIN ADSTORAGE a46 ON
    a46.UNIQUEID = PRODUCTIONDEMAND.ABSUNIQUEID
    AND a46.FIELDNAME = 'DefectTypeCode'
LEFT JOIN USERGENERICGROUP u3 ON
    u3.CODE = a46.VALUESTRING
    AND u3.USERGENERICGROUPTYPECODE = '006'
LEFT JOIN ADSTORAGE a47 ON
    a47.UNIQUEID = SALESORDERLINE.ABSUNIQUEID
    AND a47.FIELDNAME = 'cocokwarna'
LEFT JOIN ADADDITIONALDATA ad ON
    ad.NAME = a47.FIELDNAME
LEFT JOIN ADSTORAGE a48 ON
    a48.UNIQUEID = SALESORDERLINE.ABSUNIQUEID
    AND a48.FIELDNAME = 'Colorist'
LEFT JOIN ADADDITIONALDATA ad2 ON
    ad2.NAME = a48.FIELDNAME
LEFT JOIN PRODUCT p2 ON
    p2.ITEMTYPECODE = 'KGF'
    AND p2.SUBCODE01 = PRODUCTIONDEMAND.SUBCODE01
    AND p2.SUBCODE02 = PRODUCTIONDEMAND.SUBCODE02
    AND p2.SUBCODe03 = PRODUCTIONDEMAND.SUBCODE03
    AND p2.SUBCODE04 = PRODUCTIONDEMAND.SUBCODE04
LEFT JOIN ADSTORAGE a49 ON
    a49.UNIQUEID = p2.ABSUNIQUEID
    AND a49.FIELDNAME = 'NoteTas'
LEFT JOIN ADSTORAGE a50 ON
    a50.UNIQUEID = PRODUCTIONDEMAND.ABSUNIQUEID
    AND a50.FIELDNAME = 'LotBenangGreige'
    --            WHERE
    --            	PRODUCTIONDEMANDSTEP.PRODUCTIONORDERCODE = '00193878';
    --    PRODUCTIONDEMANDSTEP.PRODUCTIONDEMANDCODE = '00253783';