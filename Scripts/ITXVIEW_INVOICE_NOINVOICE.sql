...
SELECT
--	Untuk ITXVIEW_INVOICE_GRANDTOTAL3 dibuat direport langsung (ITTInvoice.rpt), logicnya sama
    COMPANYCODE,
    DIVISIONCODE,
    Invoice,
    NO_SJ,
    ...
    CASE
        ...
        AND curr = 'IDR' THEN floor(round(ROUND((JUMLAH_HARGA + biaya_tambahan) * rate, 2)* 11 / 12))
        WHEN tanggal_inv >= DATE('2024-12-31')
    ...
    CASE
        ...
        AND ppn IN (
            'S03', 'S01', 'S13'
        ) THEN floor(floor((JUMLAH_HARGA + biaya_tambahan) * rate) * 0.11)
        WHEN tanggal_inv >= DATE('2024-12-31')
        AND ppn IN (
            'S03', 'S01', 'S13', 'S05'
        ) THEN floor(round(ROUND((JUMLAH_HARGA + biaya_tambahan) * rate, 2)* 11 / 12)* 0.12)
        ELSE 0
    END AS VAT_BC,
    CASE
        WHEN curr = 'IDR' THEN 0
        ELSE round(JUMLAH_HARGA + biaya_tambahan + CASE WHEN tanggal_inv <= DATE('2024-12-31') AND ppn IN ('S03', 'S01', 'S13') THEN ROUND((JUMLAH_HARGA + biaya_tambahan), 2) * 0.11
        												WHEN tanggal_inv >= DATE('2024-12-31') AND ppn IN ('S03', 'S01', 'S13', 'S05') THEN round((((JUMLAH_HARGA) + (biaya_tambahan))* 11)/ 12, 2)* 0.12
        												ELSE 0 END, 2)
    END AS TOTAL,
    ...
    (
        ...
        PLANTINVOICE.COMPANYCODE,
            ...
            CASE
                ...
                AND SALESDOCUMENTLINE.ITEMTYPEAFICODE NOT IN (
                    'DYC', 'GYR', 'GEL'
                ) THEN GRANDTOTAL.TOTAL_YARD
                WHEN trim(SALESDOCUMENTLINE.PRICEUNITOFMEASURECODE) = 'kg'
                AND SALESDOCUMENTLINE.ITEMTYPEAFICODE IN (
                    'DYC', 'GYR', 'GEL'
