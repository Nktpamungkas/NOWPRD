DROP VIEW ITXVIEW_OUTSTANDING_MTC;
CREATE VIEW ITXVIEW_OUTSTANDING_MTC AS
SELECT
    DISTINCT *
FROM
    (
        SELECT
    		...
    		CAST(
                LPAD(MONTH(p3.STARTDATE - a2.VALUETIMESTAMP), 2, '0') + LPAD(MONTH(CURRENT_TIMESTAMP - p3.STARTDATE), 2, '0') AS INTEGER
            ) AS TOTAL_DURASI_BULAN,
            CAST(
                LPAD(DAY(p3.STARTDATE - a2.VALUETIMESTAMP), 2, '0') + LPAD(DAY(CURRENT_TIMESTAMP - p3.STARTDATE), 2, '0') AS INTEGER
            ) AS TOTAL_DURASI_HARI,
            CAST(
                LPAD(HOUR(p3.STARTDATE - a2.VALUETIMESTAMP), 2, '0') + LPAD(HOUR(CURRENT_TIMESTAMP - p3.STARTDATE), 2, '0') AS INTEGER
            ) AS TOTAL_DURASI_JAM,
            CAST(
                LPAD(MINUTE(p3.STARTDATE - a2.VALUETIMESTAMP), 2, '0') + LPAD(MINUTE(CURRENT_TIMESTAMP - p3.STARTDATE), 2, '0') AS INTEGER
            ...
            p.STATUS,
            p.CREATIONDATETIME,
            u.LONGDESCRIPTION AS ROOT_CAUSE
        FROM
            PMBREAKDOWNENTRY p
        ...
        LEFT JOIN USERGENERICGROUP u ON u.USERGENERICGROUPTYPECODE = 'BRC' AND u.CODE = p3.REASONCODECODE
        WHERE
            p.COUNTERCODE = 'PBD001'
            AND p.STATUS IN ('1', '2', '3')
            AND CAST(p.CREATIONDATETIME AS DATE) BETWEEN (CURRENT_TIMESTAMP - 2 MONTH) AND CURRENT_TIMESTAMP
--            AND p.CODE = 'BD000019348'
        ORDER BY
            p.STATUS ASC,
